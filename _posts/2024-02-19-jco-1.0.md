...
experimentally, including: native support for browser and Deno.js JavaScript
runtimes. As well as native support for compiling JavaScript code into
WebAssembly.
...
compiler toolchain built for WASI 0.1 using the [QuickJS JavaScript
engine](https://bellard.org/quickjs/). This work predates Wasm Components, but
Component support may be added in the future.
- [ComponentizeJS](https://github.com/bytecodealliance/ComponentizeJS): A
JavaScript → WASI compiler toolchain built for WASI 0.2 and Wasm Components.
This project is newer and not yet considered stable.
...
for WASI 0.2 and Wasm Components using the [SpiderMonkey JavaScript
engine](https://spidermonkey.dev).

The goal is for Jco to grow into a comprehensive toolchain which takes care of
all JavaScript and WebAssembly interactions. Jco already experimentally exposes
the functionality of ComponentizeJS via the `jco componentize` command. And in
the future we expect to add support for the [WebAssembly
registry](https://github.com/bytecodealliance/registry/tree/main) too.
