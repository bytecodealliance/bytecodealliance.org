...
experimentally, including native support for the browser. As well as native
support for compiling JavaScript code into WebAssembly.
...
JavaScript → Wasm Component compiler toolchain with full support for WASI 0.2.
This project is newer and not yet considered stable.
- [Jco](https://github.com/bytecodealliance/jco): A comprehensive WebAssembly
Component toolchain and runtime for JavaScript. This includes a Wasm Component → JS
toolchain with full support for WASI 0.2 built using the [SpiderMonkey
JavaScript engine](https://spidermonkey.dev).

The goal is for Jco to grow into a comprehensive toolchain which takes care of
all JavaScript and WebAssembly Component interactions. Jco already
experimentally exposes the functionality of ComponentizeJS via the `jco
componentize` command. And in the future we expect to add support for the
[WebAssembly registry](https://github.com/bytecodealliance/registry/tree/main)
too.
