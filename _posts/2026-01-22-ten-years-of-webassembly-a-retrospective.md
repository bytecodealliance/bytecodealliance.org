...
The road to standardization (and understated ubiquity) would require the teams behind the major browsers to unite behind an approach that Wagner looks back on as "super hacky&mdash;*hilariously* hacky." 
...
PNaCl placed the program's call stack in its own memory space&mdash;a reasonable choice for sandboxing, but one that made it difficult for JavaScript to call PNaCl code or vice versa. The connection between PNaCl and the rest of the browser relied on message passing, which required an entirely separate API surface for graphics, audio, and networking. This created an obstacle to broader adoption. If PNaCl was going to serve as the basis for a multi-browser approach to native code on the web, what standards body would govern the APIs?
